...
Without the AAR the watch app still builds against the local compile-time stub, but hardware ECG cannot run. `ECG_ON_DEMAND` will not be granted to `app.healthtrack` on a stock Galaxy Watch unless Samsung has partner-whitelisted this package.
